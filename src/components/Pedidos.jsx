...
const Pedidos = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        // Ordenar los pedidos pendientes por pickupTime
  ...
  const handleChangeStatus = async (id, newStatus) => {
    try {
      ...
      await updateDoc(orderRef, { status: newStatus }); // Actualiza en la base de datos
    } catch (error) {
  ...
  const deliveredOrders = orders
    ...
    .sort((a, b) => b.orderId - a.orderId); // Ordenar por orderId de forma decreciente
        ...
        {pendingOrders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {pendingOrders.map((order) => (
              ...
              >
                <h3 className="text-xl font-semibold text-brick mb-2">
        ...
        {deliveredOrders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {deliveredOrders.map((order) => (
              <div
                key={order.id}
                className="p-4 bg-green-100 shadow rounded-lg border border-gray-200"
              >
